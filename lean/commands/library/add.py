...
def _get_nuget_package(name: str) -> Tuple[str, str]:
    ...
    """
    generic_error = RuntimeError(f"Could not retrieve the latest version of the {name} package, please use --version")
    ...
    if package_data is None:
        raise RuntimeError(f"Could not find a package named {name} on NuGet")

    return package_data["id"], package_data["versions"][-1]["version"]


def _add_package_to_csproj(csproj_file: Path, name: str, version: str) -> None:
...
def _add_csharp(project_dir: Path, name: str, version: Optional[str]) -> None:
    ...
    _add_package_to_csproj(csproj_file, name, version)
...
def _is_pypi_file_compatible(file: Dict[str, Any], required_python_version: StrictVersion) -> bool:
    """Checks whether a file on PyPI is compatible with a certain Python version.
...
def _get_pypi_package(name: str) -> Tuple[str, str]:
    ...
    if response.status_code == 404:
        raise RuntimeError(f"Could not find a package named {name} on PyPI")

    if not response.ok:
        raise RuntimeError(f"Could not retrieve the latest version of the {name} package, please use --version")
    ...
    for version, files in pypi_data["releases"].items():
        for file in files:
    ...
    if last_compatible_version is None:
        raise RuntimeError(
            f"Could not find a version of the {name} package compatible with Python {required_python_version}")

    return name, last_compatible_version


def _add_python(project_dir: Path, name: str, version: Optional[str]) -> None:
    ...
    if version is None:
        ...
        name, version = _get_pypi_package(name)
    ...
    requirements_lines = requirements_content.splitlines() + [f"{name}=={version}"]
...
def add(project: Path, name: str, version: Optional[str]) -> None:
    ...
    if project_language == "CSharp":
        _add_csharp(project, name, version)
    else:
        _add_python(project, name, version)
